package com.example.authorizationserverac.security;
...
public class SecurityConfiguration {
    ...
    // Customiza o token jwt com informações do resource owner
    @Bean
    public OAuth2TokenCustomizer<JwtEncodingContext> jwtEncodingContextOAuth2TokenCustomizer() {
        return (context -> {
            ...
            if (auth.getPrincipal() instanceof final ResourceOwnerEntity roEntity) {
                context.getClaims().claim("resource_owner_id", roEntity.getId());
                context.getClaims().claim("email", roEntity.getEmail());
            }
        });
    }
    ...
    public RegisteredClientRepository registeredClientRepository(PasswordEncoder defaultEncoder) {
        RegisteredClient client = RegisteredClient
                .withId("1")
                .clientId("client")
                .clientSecret(defaultEncoder.encode("secret"))
                .clientAuthenticationMethod(ClientAuthenticationMethod.CLIENT_SECRET_BASIC)
                .authorizationGrantType(AuthorizationGrantType.AUTHORIZATION_CODE)
                .authorizationGrantType(AuthorizationGrantType.REFRESH_TOKEN)
                .redirectUri("http://localhost:3000/loged")
